...
struct BoopoverView: View {
    ...
    @ScaledMetric var header: CGFloat = 20
    ...
    var body: some View {
        ...
        .onAppear {
            ...
            monitor = NSEvent.addLocalMonitorForEvents(matching: .keyDown) { event in
                let keyCode = KeyMappings(rawValue: Int(event.keyCode))?.printDescription()
                print("Key down event: \(keyCode != nil ? keyCode! : "\(event.keyCode)")")
